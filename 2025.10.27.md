...
На фотографии мы можем заметить часть названия улицы, отзеркалим фото, чтобы стало понятнее. Также по надписям понимаем, что это Германия.
...
По надписи понимаем, что это Южная Корея, ищем заведение Paris Baguette, которое видно на фото в похожем на фото место.
...
К задаче приложенфайл сетевого трафика. Напишем программу, которая автоматически перебирает пакеты и оставляет только DNS-запросы, из которых извлекает части, похожие на base64, затем декодирует эти части в текст, собирает полученные куски в правильной последовательности и выводит итоговую строку.
...
В консоли браузера запустим скрипт, который перебирает набор предполагаемых API-путей, парсит ответы и ищет в них строки вида deadface{...}.
