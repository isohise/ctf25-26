...
Напишем программу, которая факторизует модуль `n` (пробное деление + Pollard Rho с проверкой Миллера–Рабина), по найденным простым считает `φ(n)`, находит приватную экспоненту `d ≡ e^(-1) mod(φ(n))`, расшифровывает `m = c^d mod(n)` и выводит флаг, декодировав `m` в байты/ASCII. 
