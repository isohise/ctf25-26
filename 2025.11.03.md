<img width="493" height="925" alt="image" src="https://github.com/user-attachments/assets/da939ae9-ee5b-4a8f-b48f-dbab11d0a6f8" />[V1t CTF 2025](https://ctf.v1t.site)
...
Далее напишем программу, которая для каждого токена берет последний бит (младший). Все эти младшие биты последовательно соединяются в одну длинную битовую строку. После получения этой строки разбивает ее на группы по восемь бит и каждую такую группу переводит в символ ASCII.
...
Откроем файл `flag.enc`, в нём видим последовательность целых чисел (остатки от деления каждого символа флага на неизвестный ключ k).

![](img/4/6.png)

Далее напишем программу, которая читает `flag.enc`, для каждого `k` строит для каждого остатка `r` набор символов `c` с `c%k==r`, отсекает несовместимые/слишком громоздкие варианты и перебором ищет строки вида `v1t{...}`.
...
В выводе программы видим множество кандидатов флага, из них выбираем корректный по смыслу и формату флаг.
...
Далее напишем программу, которая факторизует модуль `n` (пробное деление + Pollard Rho с проверкой Миллера–Рабина), по найденным простым считает `φ(n)`, находит приватную экспоненту `d ≡ e^(-1) mod(φ(n))`, расшифровывает `m = c^d mod(n)` и выводит флаг, декодировав `m` в байты/ASCII. 
...
Напишем программу для решения 1 части задания. Она по данным из LCG и одному значению `next` перебирает подходящие состояния, отматывает генератор назад до исходного зерна, берёт из него ключ `SHA256(seed)`, пробует расшифровать AES-ECB и, если паддинг/текст корректны, выводит найденный `seed`, ключ и открытый текст.
...
Далее для второй части. Программа ниже берет hex-строку, строит обратную таблицу к побитовой функции `F`, затем восстанавливает первый байт как `invF(y0)`, а каждый следующий — как `invF(yi) ^ y_{i-1}`, после чего выводит исходные байты и их ASCII-представление.
...
В задании видим эмодзи, значит, текст могли спрятать невидимыми символами рядом с ним. Напишем программу, которая берет строку с эмодзи из задания, вытаскивает невидимые emoji-теги, переводит их в байты, фильтрует по печатаемым символам и склеивает сообщение.
